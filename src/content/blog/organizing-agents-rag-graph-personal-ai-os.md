...
SHU makes this a small, explicit graph instead of a paragraph. `.claude/graph.yaml` encodes the system's real workflows as nodes (agents) and edges (transitions), and every edge carries a `gate`. One edge looks like this:
...
The full shape of a workflow (including where it stops) is easier to read as a picture than as more YAML:
