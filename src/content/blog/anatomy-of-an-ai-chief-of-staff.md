...
**4. Structured memory, write-back.** Facts and decisions that persist across sessions. Every agent is expected to *write* to it, not just read from it. A memory store you only query is a search engine, not a system that accumulates judgment.
