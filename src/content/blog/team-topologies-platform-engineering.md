...
The defining characteristic is outcome ownership, not technology ownership. A stream-aligned team owns a business metric, not a tier in the stack. If the checkout conversion rate drops, this team is responsible. The diagram above is the whole point: no separate box for "the backend team" or "the payment team" to hand off to. The payment logic inside `checkout-service` is written and owned by the same seven people who own the outcome, not routed to a different team's backlog.
...
```mermaid
sequenceDiagram
    participant EN as Enabling Team
    participant ST as Stream-Aligned Team
    EN->>ST: Engage — define boundaries, set up pipeline, teach a pattern
    Note over EN,ST: 4-6 weeks, time-boxed from day one
    EN->>ST: Knowledge transfer
    EN-->>ST: Step back
    Note over ST: Team runs independently — no standing dependency on EN
```

The engagement has a defined end from the start, not an open-ended "let us know if you need anything." That's what keeps it Enabling instead of a support queue in disguise.
...
```mermaid
flowchart LR
    subgraph ST["Stream-Aligned Team"]
        C["checkout-service"]
    end
    subgraph CS["Complicated Subsystem Team: Risk Engineering"]
        API["getQuickRiskDecision(customerId, amount)\nsimple public surface"]
        INT["Base risk × market adjustment × time decay\ndeep domain logic — hidden"]
        API --> INT
    end
    C -->|"X-as-a-Service"| API
    INT -.->|"never called directly"| C

    style API fill:#2b6cb0,color:#fff
    style INT fill:#6b46c1,color:#fff
```

Everything below the API line (market adjustments, time decay, confidence intervals) is invisible to `checkout-service`, on purpose. The stream team gets a decision, not a reason to understand credit risk modeling.
...
| Capability | What it provides | How teams adopt it |
|---|---|---|
| **CI/CD** | Golden path deployment pipeline | Self-service via pipeline-as-code template |
| **Container Platform** | Kubernetes with security defaults | Self-service namespace creation |
| **Observability** | Pre-configured Grafana + distributed tracing | Auto-instrumented for Spring Boot services |
| **Secret Management** | Vault integration with automatic rotation | Sidecar injection — zero code changes required |

The adoption column is the one that actually measures whether the platform is working. "Self-service" and "zero code changes required" aren't just implementation details. They're the test. If adopting a platform capability requires filing a ticket or asking the platform team for help, it's not a platform yet, it's a service desk with extra steps.
